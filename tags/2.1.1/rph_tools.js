...
Array that holds all of the user name settings. The settings itself are
 ...
 "pingUrl":    URL to the audio source for pinging
...
var scriptSettings = {
  "pings"     : "",
  "pingUrl"  : "http://chat.rphaven.com/sounds/boop.mp3",
  "color"     : "#000",
  "highlight" : "#FFA",
  "flags"     : 0,
    ...
    '<div id="settingsContainer" style="height: 470px; width: 100%; overflow: auto; background: rgb(51, 51, 51); padding: 10px; border-radius: 5px; font-size: 0.8em;">' +
     ...
     '<div id="diceForm" style="display:none;">' +
      ...
      '<div id="aboutHelpForm" style="display:none;">' +
        ...
        '<a href="https://openuserjs.org/scripts/shuffyiosys/RPH_Tools#troubleshooting" target=_blank">Troubleshooting Tips</a> | RPH Tools 2.1.1</p><br>' +
...
$(function() {
  ...
  if (typeof(storage) != "undefined") {
    if (localStorage.getItem("chatSettings") !== null) {
      ...
      if (scriptSettings.ping_url !== undefined){
        /* Update to the new name*/
        scriptSettings.pingUrl = scriptSettings.ping_url;
        scriptSettings.ping_url = undefined;
      }

      snd = new Audio(scriptSettings.pingURL);
      $('#im-sound').children("audio").attr('src', scriptSettings.pmPingUrl);
      console.log("RPH Tools - Laoded chat settings: ", scriptSettings);
    }
...
function SetUpToolsDialog() {
  settingsTool.button.click(function() {
    ...
    else
    {
      if ((scriptSettings.flags & 1) > 0) {
        console.log('RPH Tools - Chat settings were changed', scriptSettings);
  ...
  $('#settingsButton').click(function() {
    console.log('RPH Tools - Chat settings', scriptSettings);
...
function ChatSettingsSetup() {
  ...
  $('#pingURL').blur(function() {
    ...
    if (testPingURL(pingUrl) === false) {
      mark_problem('pingURL', true);
      validSettings = false;
    }
    else{
      scriptSettings.pingUrl = pingUrl;
      snd = new Audio(scriptSettings.pingUrl);
      scriptSettings.flags |= 1;
...
function populateSettingsDialog() {
  $('textarea#pingNames').val(scriptSettings.pings);
  $('input#pingURL').val(scriptSettings.pingUrl);
  $('input#pingTextColor').val(scriptSettings.color);
  $('input#pingHighlightColor').val(scriptSettings.highlight);
